...
class Dashboard:
    ...
    def get_queue_stats(self) -> Dict:
        ...
        pending = self.run_query("SELECT COUNT(*) FROM mail_que WHERE processed = 0;")
        pending_count = int(pending[0][0]) if pending and pending[0] else 0

        # Get completed emails (processed = 1)
        completed = self.run_query("SELECT COUNT(*) FROM mail_que WHERE processed = 1;")
        ...
        return {'pending': pending_count, 'completed': completed_count, 'total': total_count}
    ...
    def get_trained_model_stats(self) -> Dict:
        ...
        trained = self.run_query("SELECT COUNT(*) FROM email_hashes WHERE trained = 1;")
        ...
        return {'total_trained': total_trained, 'by_class': trained_by_class}
    ...
    def display(self):
        ...
        # Header
        print(
            f"{Colors.BOLD}{Colors.WHITE}╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.WHITE}║{Colors.CYAN}                                    PostMaster Email Classification Dashboard - {self.get_timestamp()}{Colors.WHITE}                                                      ║{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.WHITE}╚══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝{Colors.NC}"
        )
        ...
        print(
            f"{Colors.BOLD}{Colors.BLUE}┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.BLUE}│{Colors.WHITE} 📊 QUEUE STATUS{Colors.BLUE}                                                                                                                │{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.BLUE}├─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤{Colors.NC}"
        )
        ...
        if total_queued > 0:
            ...
            print(
                f"{Colors.BLUE}│{Colors.NC}  📨 {Colors.WHITE}Total Emails in Queue:{Colors.NC} {self.format_number(total_queued):>10}"
            )
            print(
                f"{Colors.BLUE}│{Colors.NC}     {self.draw_bar(completed_percent, 50, Colors.GREEN)} {completed_percent}% Completed ({self.format_number(completed_queue)})"
            )
            print(
                f"{Colors.BLUE}│{Colors.NC}     {self.draw_bar(pending_percent, 50, Colors.YELLOW)} {pending_percent}% Pending ({self.format_number(pending)})"
            )
        else:
        ...
        print(
            f"{Colors.BOLD}{Colors.MAGENTA}┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.MAGENTA}│{Colors.WHITE} 🧠 TRAINING STATUS{Colors.MAGENTA}                                                                                                           │{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.MAGENTA}├─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤{Colors.NC}"
        )
        ...
        if total_train_completed > 0:
            print(
                f"{Colors.MAGENTA}│{Colors.NC}  📊 {Colors.WHITE}Total Trained:{Colors.NC} {self.format_number(total_train_completed)} emails"
            )
        ...
        print(
            f"{Colors.BOLD}{Colors.CYAN}│{Colors.WHITE} 🤖 MODEL STATISTICS{Colors.CYAN}                                                                                                             │{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.CYAN}├─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤{Colors.NC}"
        )
        print(
            f"{Colors.CYAN}│{Colors.NC}  🎓 {Colors.WHITE}Emails in Training Set:{Colors.NC} {self.format_number(trained_stats['total_trained']):>10}"
        )
        ...
        print(
            f"{Colors.BOLD}{Colors.GREEN}┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.GREEN}│{Colors.WHITE} 📁 FOLDER DISTRIBUTION{Colors.GREEN}                                                                                                         │{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.GREEN}├─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤{Colors.NC}"
        )
        ...
        print(
            f"{Colors.BOLD}{Colors.YELLOW}┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.YELLOW}│{Colors.WHITE} 💚 SYSTEM HEALTH{Colors.YELLOW}                                                                                                               │{Colors.NC}"
        )
        print(
            f"{Colors.BOLD}{Colors.YELLOW}├─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤{Colors.NC}"
        )
        ...
        if has_processed:
            print(
                f"{Colors.YELLOW}│{Colors.NC}  📋 mail_que:       {Colors.GREEN}Active (with processed tracking) ✓{Colors.NC}"
            )
        else:
        ...
        if log_exists:
            ...
            print(
                f"{Colors.YELLOW}│{Colors.NC}  📝 Processing Log:  {Colors.GREEN}Active ({self.format_number(log_count_value)} records) ✓{Colors.NC}"
            )
        else:
        ...
        if has_trained:
            ...
            print(
                f"{Colors.YELLOW}│{Colors.NC}  🏷️  email_hashes:   {Colors.GREEN}Active ({self.format_number(trained_value)} trained) ✓{Colors.NC}"
            )
        else:
...
Examples:
    ...
    parser.add_argument('--config', '-c', type=str, default=None, help='Path to configuration YAML file')
    parser.add_argument('--interval', '-i', type=int, default=5, help='Update interval in seconds (default: 5)')
    parser.add_argument('--once', '-o', action='store_true', help='Run once and exit (single snapshot)')
