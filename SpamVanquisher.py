...
class PostOffice:
    ...
    def fetch_emails(self, mailbox, email_type):
        ...
        for email_id in tqdm(email_ids, desc=f"Fetching  ({email_type})", unit="email", colour="green"):
    ...
    def process_inbox(self, model_file, vectorizer_file):
        ...
        def postman():
            while self.running:
                ...
                for email_id in tqdm(email_ids, desc="Processing Inbox", unit="email", colour="blue"):
                    result, msg_data = mail.fetch(email_id, "(RFC822)")
                    if result == "OK":
                        msg = email.message_from_bytes(msg_data[0][1])
